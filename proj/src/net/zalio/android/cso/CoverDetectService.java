...
public class CoverDetectService extends Service implements SensorEventListener {
	...
	public void onSensorChanged(SensorEvent event) {
		if(event.sensor.getType() == mSensorManager.getDefaultSensor(Sensor.TYPE_GRAVITY).getType()){
	...
	private void onRotation(int orientation){
		MyLog.i(TAG, "onRotation: " + orientation);
		this.mCurrentRotation = orientation;
	}
	
	private int processOrientation(SensorEvent event){
        ...
        if (LOG) {
            MyLog.v(TAG, "Raw acceleration vector: "
                    + "x=" + x + ", y=" + y + ", z=" + z
        ...
        if (now < then
                ...
                || (x == 0 && y == 0 && z == 0)) {
            if (LOG) {
                MyLog.v(TAG, "Resetting orientation listener.");
            }
            reset();
            skipSample = true;
        } else {
            ...
            if (LOG) {
                MyLog.v(TAG, "Filtered acceleration vector: "
                        + "x=" + x + ", y=" + y + ", z=" + z
                        + ", magnitude=" + FloatMath.sqrt(x * x + y * y + z * z));
            }
            skipSample = false;
        ...
        if (!skipSample) {
            ...
            if (magnitude < NEAR_ZERO_MAGNITUDE) {
                if (LOG) {
                    MyLog.v(TAG, "Ignoring sensor data, magnitude too close to zero.");
                }
                ...
                if (Math.abs(tiltAngle) > MAX_TILT) {
                    if (LOG) {
                        MyLog.v(TAG, "Ignoring sensor data, tilt angle too high: "
                                + "tiltAngle=" + tiltAngle);
                    }
                    clearPredictedRotation();
                } else {
                    ...
                    if (isTiltAngleAcceptable(nearestRotation, tiltAngle)
                        ...
                        if (LOG) {
                            MyLog.v(TAG, "Ignoring sensor data, no predicted rotation: "
                                    + "tiltAngle=" + tiltAngle
                                    + ", orientationAngle=" + orientationAngle);
                        }
                        clearPredictedRotation();
        ...
        if (LOG) {
            MyLog.v(TAG, "Result: currentRotation=" 
        ...
        if (mProposedRotation != oldProposedRotation && mProposedRotation >= 0) {
            if (LOG) {
                MyLog.v(TAG, "Proposed rotation changed!  proposedRotation=" + mProposedRotation
                        + ", oldProposedRotation=" + oldProposedRotation);
            }
            // mOrientationListener.onProposedRotationChanged(mProposedRotation);
            //TODO
            onRotation(mProposedRotation);
        }
